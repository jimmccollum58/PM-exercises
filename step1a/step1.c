...
// Function prototypes
uint32_t* map (char* arrayArg[], uint32_t arraySize, uint32_t (*func) (char *));
uint32_t mapFunc (char* arg);
...
int main(void)
{
	...
	result = reduce (pTransformedArray, 9, reduceFunc);
...
uint32_t * map (char* arrayArg[], uint32_t arraySize, uint32_t (*func) (char *))
{
	...
	uint32_t *retArray = (uint32_t *) malloc (arraySize * sizeof (char*));
